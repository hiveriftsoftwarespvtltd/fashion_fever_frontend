      ...
      try {
        ...
        if (response.success && isMounted) {
          ...
          if (Array.isArray(payload)) {
            // Support both direct product list and nested { product } objects
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-16 flex flex-col items-center justify-center">
        ...
        <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Loading Trending Products...</p>
      </div>
    );
  }

  // If no products, do not render this section
  if (products.length === 0) return null;

  return (
    <section className="bg-white py-16 font-outfit border-b border-gray-100">
      <div className="container mx-auto px-4 md:px-8 max-w-[1600px]">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-10 gap-4">
          <div className="text-left">
            <span className="text-xs font-bold text-primary uppercase mb-2 block tracking-normal">Hot Right Now</span>
            <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 uppercase italic">Trending Products</h2>
          </div>
