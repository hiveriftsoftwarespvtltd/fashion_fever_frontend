...
const CoursePlayer = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col text-left">
      {/* Header */}
      <header className="h-16 bg-black/50 border-b border-white/10 flex items-center justify-between px-6 backdrop-blur-md">
         ...
         <div className="flex items-center gap-4">
            ...
            <button className="bg-primary text-white px-6 py-2 rounded-lg text-[10px] font-bold uppercase shadow-xl shadow-primary/20 cursor-pointer">
               Next Lesson
            </button>
         </div>
      </header>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-grow flex flex-col bg-black">
           <div className="flex-1 relative group cursor-pointer overflow-hidden min-h-[300px]">
              <img 
           ...
           {/* Video Info Tabs */}
           <div className="bg-gray-900 border-t border-white/5 p-8 text-left">
              <div className="flex gap-8 mb-8 border-b border-white/5 pb-4">
                 <button className="text-primary text-[10px] font-bold uppercase relative pb-4 cursor-pointer">
                    Overview
                    <div className="absolute bottom-0 left-0 w-full h-[2px] bg-primary"></div>
                 </button>
                 <button className="text-white/40 text-[10px] font-bold uppercase hover:text-white transition-all pb-4 cursor-pointer">Notes</button>
                 <button className="text-white/40 text-[10px] font-bold uppercase hover:text-white transition-all pb-4 cursor-pointer">Q&A</button>
              </div>
              <div className="max-w-3xl">
                 ...
                 <div className="flex gap-4">
                    <button className="flex items-center gap-2 text-white/60 hover:text-white transition-all text-[10px] font-bold uppercase bg-white/5 px-4 py-2 rounded-xl cursor-pointer">
        ...
        {/* Sidebar (Playlist) */}
        <div className="w-full lg:w-[400px] bg-gray-900 border-l border-white/10 flex flex-col h-full overflow-y-auto">
           ...
           <div className="flex-grow overflow-y-auto">
              {lessons.map((lesson) => (
                <button 
                  ...
                  className={`w-full p-6 flex items-start gap-4 transition-all hover:bg-white/5 border-b border-white/5 text-left cursor-pointer ${activeLesson === lesson.id ? 'bg-primary/10' : ''} ${lesson.locked ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                   <div className="mt-1">
                      {lesson.completed ? (
