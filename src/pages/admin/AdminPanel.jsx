...
const AdminPanel = () => {
  ...
  const userColumns = [
    ...
    { header: 'System role', render: (user) => <span className="px-3 py-1 rounded-lg bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-wider">{user.role}</span> },
    {
      header: 'Status',
      render: (user) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold uppercase text-gray-400">{user.isActive && !user.isDeleted ? 'Active' : 'Banned'}</span>
        </div>
      )
    },
    { header: 'Registration', render: (user) => <div className="flex flex-col"><span className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{new Date(user.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span><span className="text-[10px] uppercase font-bold text-gray-400">Onboarded</span></div> },
    {
      header: 'Actions',
      render: (user) => (
  ...
  const influencerColumns = [
    ...
    { header: 'Commission', render: (inf) => <span className="px-3 py-1 rounded-lg bg-pink-500/10 text-pink-500 text-[10px] font-bold uppercase ">{inf.commissionRate}%</span> },
      ...
      render: (slab) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase">Slab ID: {slab._id}</span>
          </div>
        </div>
      )
    },
    {
      ...
      render: (slab) => (
        <div className="flex flex-col">
          ...
          <span className="text-[10px] font-bold text-gray-400 uppercase">Min to Max Sales</span>
        </div>
      )
    },
    {
      ...
      render: (slab) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold uppercase text-gray-400">{slab.isActive ? 'Active' : 'Inactive'}</span>
        </div>
      )
    },
    {
  ...
  const couponColumns = [
    ...
    { header: 'Status', render: (cp) => <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${cp.isActive ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>{cp.isActive ? 'Active' : 'Expired'}</span> },
    {
      header: 'Actions',
      render: (cp) => (
        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
      ...
      render: (cat) => (
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold uppercase text-gray-400">/{cat.slug}</span>
          </div>
        </div>
      )
    },
      ...
      render: (cat) => (
        <div className="flex flex-wrap gap-1">
          {cat.tags && cat.tags.length > 0 ? (
            ...
            <span className="text-gray-400 text-[10px] font-bold uppercase">No tags</span>
          )}
        </div>
      )
    },
      ...
      render: (order) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase">Payment: {order.paymentMethod}</span>
          </div>
        </div>
      )
    },
    { header: 'Status', render: (order) => <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${order.status === 'delivered' ? 'bg-green-500/10 text-green-500' : 'bg-orange-500/10 text-orange-500'}`}>{order.status}</span> },
      ...
      render: (product) => (
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase">Code: {product._id?.substring(18)}</span>
          </div>
        </div>
      )
    },
      ...
      render: (product) => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-semibold uppercase text-gray-400">{product.status || 'ACTIVE'}</span>
          </div>
        );
      }
    },
            ...
            <div className="space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className={`px-4 lg:px-6 py-3 lg:py-4 rounded-2xl lg:rounded-[24px] border shadow-sm transition-colors duration-300 ${isDarkMode ? 'bg-gray-800 border-white/5' : 'bg-white border-gray-100'}`}>
                  <p className="text-[10px] font-bold uppercase text-gray-400 mb-0.5">Total {activeTab === 'commission-slabs' ? 'Slabs' : activeTab}</p>
