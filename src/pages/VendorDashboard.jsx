...
import { getVendorDetails, getVendorProducts, getVendorCategories, editVendorDetails, createCategory, deleteCategory, updateCategory, createProduct, updateProduct, deleteProduct, getProductDetails, deleteProductVariant } from '../api/vendorService';
import {
  LayoutDashboard,
  Package,
  ShoppingCart,
...
const VendorDashboard = () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (initialData.variants.length === 0) {
          initialData.variants = [{ sku: '', price: '', salesPrice: '', stock: '', attributes: { color: '', size: '' }, thumbnail: null, images: [] }];
        }
  ...
  const closeProductModal = async () => {
    // Skip warning if in view mode
    if (isViewingProduct) {
      setShowProductModal(false);
      return;
    }

    // Check for changes (simplified check using JSON stringify)
  ...
  const handleDeleteVariant = async (vIdx) => {
    ...
    // If it's an existing variant in DB
    const result = await Swal.fire({
      ...
      customClass: {
        ...
        confirmButton: 'bg-red-500 px-6 py-2.5 rounded-xl font-bold uppercase text-xs',
        cancelButton: 'bg-gray-100 text-gray-800 px-6 py-2.5 rounded-xl font-bold uppercase text-xs'
      }
    });
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {categoriesLoading ? (
                        ...
                        <tr key={cat._id} className="hover:bg-gray-50/50 transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${cat.isActive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
            ...
            <div className="p-8 space-y-10 overflow-y-auto flex-1">
              {isViewingProduct ? (
                <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  {/* Variants Section */}
                  <div className="space-y-6">
                    <h3 className="text-sm font-bold text-gray-900 uppercase  border-l-4 border-primary pl-3">Inventory & Variants ({productForm.variants.length})</h3>
                          ...
                          <div className="flex flex-col md:flex-row gap-8">
                            ...
                            <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-6 content-center">
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-sm font-bold text-primary">₹{v.salesPrice} <span className="text-xs text-gray-300 line-through ml-1">₹{v.price}</span></p>
                              </div>
                              <div className="space-y-1">
